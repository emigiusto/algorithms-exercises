...
public class Grapho {
   ...
   /*  private boolean valid = true;
     ...
     */
    public boolean addEdge(int v, int w, boolean conflict) {
        validateVertex(v);
        validateVertex(w);
        E++;
        adj[v].add(w);
        adj[w].add(v);
        boolean valid = true;
        if (conflict) { // Vertexs have different color
            if ((vertexColor[v] != 0 || vertexColor[w] != 0) && vertexColor[v] != vertexColor[w]) {
                if (vertexColor[v]==1) {
                    vertexColor[w]=2;
                } else if (vertexColor[w]==1){
                    vertexColor[v]=2;
                } else if (vertexColor[v]==2){
                    vertexColor[w]=1;
                }  else if (vertexColor[w]==2){
                    vertexColor[v]=1;
                } else {
                }
            } else { valid = false; }
        } else { //Vertexs have same color
            if ((vertexColor[v] != 0 || vertexColor[w] != 0) && vertexColor[v] == vertexColor[w]) {
                if (vertexColor[v]==1) {
                    vertexColor[w]=1;
                } else if (vertexColor[w]==2){
